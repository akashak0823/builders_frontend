...
const Dashboard: React.FC = () => {
    ...
    return (
        <div className="space-y-6 max-w-[1600px] mx-auto">
            ...
            <div className="flex items-center bg-navy-900 rounded-2xl p-6 shadow-2xl relative overflow-hidden">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <Card 
                    ...
                    className="relative overflow-hidden group cursor-pointer hover:shadow-xl hover:-translate-y-1 transition-all duration-300 border border-transparent hover:border-gold-400"
                >
                    <div className="absolute right-0 top-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
                        ...
                        {stats.recentInvoices.length === 0 ? (
                            ...
                            stats.recentInvoices.map((invoice) => (
                                <div 
                                    key={invoice._id} 
                                    onClick={() => navigate('/invoices')}
                                    className="flex items-center justify-between p-3 rounded-lg bg-light-100 hover:bg-white hover:shadow-md transition-all border border-transparent hover:border-slate-200 cursor-pointer"
                                >
                                    <div className="flex items-center">
                                        ...
                                        <div>
                                            <p className="font-bold text-navy-900 text-sm">{invoice.invoiceNumber}</p>
                                            <p className="text-xs text-bluegrey-500">{invoice.customer.name}</p>
                                        </div>
                                    </div>
                                    <div className="text-right">
                                        <p className="font-bold text-navy-900 text-sm">₹{invoice.grandTotal.toLocaleString()}</p>
                                        <span className="text-[10px] font-bold text-accent-green bg-accent-green/10 px-2 py-0.5 rounded-full">PAID</span>
                                    </div>
                                </div>
                            ))
                        )}
                ...
                <Card title="Quick Actions">
                    <div className="grid grid-cols-2 gap-4 h-[280px]">
                        <button
                            ...
                            className="h-full p-6 rounded-xl bg-navy-900 text-white hover:bg-navy-800 hover:-translate-y-1 transition-all shadow-lg flex flex-col items-center justify-center group"
                        >
                        ...
                        <button
                            onClick={() => navigate('/products')}
                            className="h-full p-6 rounded-xl bg-white border border-slate-200 text-navy-900 hover:border-gold-400 hover:shadow-lg hover:-translate-y-1 transition-all flex flex-col items-center justify-center group"
                        >
